...
#define VECTOR_REMOVE_BY_VALUE(VEC, VALUE) do { \
    ...
    ssize_t VECTOR_REMOVE_BY_VALUE_i = VECTOR_FIND(VECTOR_REMOVE_BY_VALUE_vec, VECTOR_REMOVE_BY_VALUE_v); \
    if (VECTOR_REMOVE_BY_VALUE_i != VECTOR_INVALID_INDEX) { \
        VECTOR_REMOVE(VECTOR_REMOVE_BY_VALUE_vec, (size_t)VECTOR_REMOVE_BY_VALUE_i); \
    } \
} while (0)
